##########################################
## Experiment on varying tau magnitude
##########################################
...
for (i in 1:length(params)){
  for (j in 1:reps){
    ...
    if (param == 'linear homogeneous'){
      # linear
      mu = function(x){1 + g(x[,4]) + x[,1]*x[,3]} # xbcf linear
      h = function(x){1.1*pnorm(3*mu(x) / sd(mu(x)) - 0.5*x[,1] - 3)  - 0.15 + runif(nrow(x))/10}
      # homogenerous
      tau <- function(x) {rep(3, nrow(x))}
    }
      ...
      h = function(x){1.1*pnorm(3*mu(x) / sd(mu(x)) - 0.5*x[,1] - 3)  - 0.15 + runif(nrow(x))/10}
      # heterogeneous
      tau = function(x){1 + 2 * x[,2] * x[,5]}
    }
    else if (param == 'non-linear homogeneous'){
      # non-linear
      mu = function(x){ -6 + g(x[,4]) + 6*abs(x[,3] - 1) }
      h = function(x){1.1*pnorm(3*mu(x) / sd(mu(x)) - 0.5*x[,1]) - 0.15 + runif(nrow(x)) / 10}
    ...
    # Some how xbart.multinomial does not work well on jalapeno
    # fitz = XBART.multinomial(y = z, num_class = 2, X = x, Xtest = x, p_cateogrical = 2, num_sweeps=100)
    # ps = apply(fitz$yhats_test, c(2, 3), mean)[,2]
    ...
    # xbcf.gp.tau = t(apply(xbcf.gp$tau.adjusted, 1, function(x) rnorm(length(x), x, sigmahat))) 
    ...
    #sate per draw
    # ce_xbcf_gp$ate = colMeans(xbcf.gp.tau)
...
for (param in params){
  ...
  temp <- data.frame(xbcf = unlist(xbcf), xbcf_gp = unlist(xbcf_gp), 
